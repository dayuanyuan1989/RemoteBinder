...
uint ScrollText::waitForStartTime() const
{
    return m_waitForStartTime;
}

void ScrollText::setWaitForStartTime(uint time)
{
    m_waitForStartTime = time;
    updateText();
    update();
}
...
void ScrollText::updateText()
{
    ...
    if(scrollEnabled)
    {
        ...
        timer.start();
 //       timer.start(m_waitForStartTime);
    }
    else
    {
